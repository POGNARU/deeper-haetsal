...
각 인사이트 리포트는 성찰 글 + 「이번 주 손으로 지은 것들」 공개 링크로 구성되며,
필요 시 **「AI를 디자인에 쓴 사례」** 같은 추가 공유 섹션(카드·프롬프트·아이콘)을 덧붙입니다.

- **인사이트 리포트 파일명은 `YYMMDD.html`로 통일**합니다. 새 회고는 같은 규칙으로 떨어뜨리면 목록에 한 줄씩 쌓입니다 (5월 회고분도 같은 형식으로 재작성해 추가 예정).
- **과제**는 완성되는 대로 카드 하나씩 `assignment/`에 더해 갑니다.
