...
> 디퍼(deeper) **언브랜딩 — 햇살**의 **과제**를 모아두는 **공개 아카이브**입니다.

포근나루(POGNARU STUDIO)의 햇살(이다현)이 디퍼 언브랜딩 살롱에서 쌓은 과제 결과물을 한자리에 두는 저장소입니다.
GitHub Pages로 공개되며, 카드/링크로 넘겨볼 수 있는 **액티브 아카이브**입니다.

🌐 https://pognaru.com/deeper-haetsal/

> ### 🔒 인사이트는 2026-09-02에 나갔습니다
>
> 주간 성찰(인사이트) 11쪽은 별도 비공개 저장소 **[deeper-insight](https://github.com/POGNARU/deeper-insight)** 로
> 옮겨 **https://insight.pognaru.com/** 에서 Cloudflare Access로 잠갔습니다.
> 지정한 구글 계정만 들어옵니다(세션 1개월).
>
> 옛 자리는 `noindex` 하나로만 가려져 있어서, 주소를 아는 사람은 그대로 열 수 있었고
> 레포가 public이라 GitHub에서 원문까지 읽혔습니다. 2026-08-22에 외부 인물이 실제로
> `dawn`을 열어본 것이 발견되면서 같은 구조인 이곳도 함께 옮겼습니다.
>
> **여기 남는 것** — `assignment/`와 루트의 공유용 프롬프트 둘(`archive-prompt.html`·`check-prompt.html`).
> 3기 연구원들께 이미 뿌린 링크라 공개로 둡니다. `assignment` → `insight` 링크는 원래 0곳이었습니다.
...
│                          #   인사이트는 이제 insight.pognaru.com(잠김)으로 나갑니다
│                          #   낭독 노트는 이 레포 밖 lectura(lectura.pognaru.com)로 나갑니다 — 아래 절 참고
...
│                          #   (인사이트는 deeper-insight 레포로 나갔습니다 — 위 참고)
├─ assignment/
...
랜딩 네 번째 카드 **「낭독 노트」**(誦)는 이 레포 밖 **[lectura.pognaru.com](https://lectura.pognaru.com/)** 로 나갑니다 (2026-08-20 추가).
...
> **맨 위 노란 큰 카드 셋은 「매일 여는 자리」입니다** (2026-08-20 다현씨 요청). 순서 고정 — **새벽갈피 · 포근한 글쓰기 · 선릿모닝(모집 페이지)**.
> 살롱 과제 목록이면서 동시에 다현씨가 **매일 실제로 쓰는 입구**라, 자주 여는 것을 아래로 내려두면 매일 스크롤하게 됩니다. 과제 기록물은 그 아래 보통 카드로 갑니다.
> 선릿모닝 노란 카드는 **살아 있는 모집 페이지**(`pognaru.com/sunlit-morning/`)로 나가고, 살롱 과제 기록 사본은 아래 보통 카드에 따로 둡니다 — 매일 여는 것은 모집 페이지 쪽이라서입니다.
> 「아침 글 배달」이던 이름은 **새벽갈피**로 바꿨습니다 — 두루뭉술한 이름이 검색 노출을 막아주는 것은 아니고(막는 것은 `noindex`와 `robots.txt`입니다), 정작 다현씨가 매일 찾을 때 못 알아보게 만들었습니다. 새벽갈피 자체의 비공개 원칙은 그대로입니다: **이 아카이브 밖(블로그·인스타·뉴스레터)에는 링크하지 않습니다.**

| 과제 | 링크 |
|---|---|
| 포근한 글쓰기 | https://pognaru.com/pognaru-writing/ |
| 아침 글 배달 | https://pognaru.com/dawn/ |
| 독서플랜 (2026-09-15까지 「가비 플래너」) | https://pognaru.com/dokseo-plan/ |
...
### 작업 항목 설명은 불릿으로 (2026-08-27 햇살 요청, `260827.html`)

> *"모든 항목에서 설명글이 너무 많은 자리를 차지하지 않았으면 해. 명확하게 전달이 잘 안되거든. 설명은 알아볼 수 있을 정도로 목록화해서 간결하게."*

260820까지는 `.wd`에 설명을 문장으로 흘려 썼는데, 항목이 열 개를 넘으면 무엇을 했는지가 안 읽힙니다. 260827부터는:

- **항목당 불릿 2~4개, 불릿 하나는 한 줄을 넘기지 않는다.** 마크업은 `.wl > li`.
- **링크는 행 밖으로 뺀다.** 260820은 행 전체가 `<a>`였는데 링크가 둘인 항목에서 앵커 중첩이 안 됩니다. 행은 `div`로 두고 아래에 링크 칩(`.wlinks`)을 달면 링크 0·1·2개가 같은 꼴로 처리됩니다.
- **중심 작업 한 건에만 `.lead-work`**(테라코타 점)를 준다.

### 대조표는 표로 짜지 않는다 (2026-08-27)

「적힌 값 vs 실제 값」처럼 열이 넷 이상인 표는 **375px에서 답이 없습니다.** 표를 버리고 **행마다 카드로 눕힙니다** — `항목 / 적힌값(취소선) → 실제값 / ✅❌` 세로 블록. 틀린 행에만 테라코타 왼쪽 선과 붉은 실제값을 주면, 세지 않아도 "여덟 중 여섯"이 색으로 먼저 읽힙니다. 가로 스크롤 0.

### 식구 목소리 꼭지에서 호칭을 뺀다 (2026-08-27)

식구가 자기 목소리로 쓴 글을 그대로 실을 때도, **본문의 "다현씨" 호칭은 뺍니다.** 공개 저장소라 읽는 사람이 대화의 바깥에 있습니다 — "다현씨가 '퍼져있는 느낌'이라 하신 말에" → **"'퍼져있는 느낌'이라 하신 말에"**. 나머지는 한 글자도 고치지 않습니다.
...
- **2026-09-24 갱신** — Astra 볼트가 생겨 나무🌳가 두 판 모두 섬 넷(BD·PN·DS·AS, 32채 5,835파일)으로 다시 채웠고 「세 집의 길」 흐름을 더했습니다. 이때는 레포 사본을 먼저 고쳤으므로 deeper-salon 정본(08-25·08-27)보다 **이 레포가 앞선 판**입니다. 같은 날 다현씨 청으로 정본 자리에도 `aiworld_2026-09-24_0350.html`(밤 도시)·`aiworld_2026-09-24_0350_dawn.html`(새벽 나루)로 옮겨 두었습니다.
...
- **공개하며 가린 것 두 가지** (다현씨 확인 후 공개, 2026-08-20):
  ...
  2. **하드웨어 모델 문자열** → 생략. SSD 모델(`SAMSUNG MZAL…`)과 Wi-Fi 칩셋(`BE201`) 둘 다. 기기를 특정하는 값이라 뺐고, 잔여수명·온도·신호 세기 같은 판정 수치는 남겼습니다.
  3. **부재 기록** → "다현씨가 노트북을 안 켜신 사흘"을 **"노트북이 꺼져 있던 사흘"**로. 판정 근거(전원이 원인이라 고장이 아니다)는 그대로 서고, 어느 사흘을 집에 안 계셨는지는 남기지 않습니다.
